...
def fitness(chromosome, tasks_df, teams_df):
    ...
    # Penalidade se mid-level tiver mais tarefas que seniors ou specialists
    mid = tasks_by_level['Mid-level']
    senior = tasks_by_level['Senior']
    specialist = tasks_by_level['Specialist']

    if mid > senior:
        penalty += 10 * (mid - senior)
    if mid > specialist:
        penalty += 10 * (mid - specialist)

    # Penalidade por desbalanceamento muito grande na quantidade de tarefas
    std_task_distribution = np.std(list(task_counts.values()))
    penalty += 5 * std_task_distribution
